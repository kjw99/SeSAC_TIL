...
- 집합
    - 0개 이상 원소에 대해 중복을 허용하지 않으면서 순서 없이 저장하는 컨테이너 자료형. {} 를 사용해 생성.
    - 중복된 원소는 제거됨.
    - `number = {1, 2, 3, 4}` 이런 느낌으로 생성. set 타입.
    - 집합의 원소는 기본 자료형과 변경 불가능한 컨테이너만 허용. 그래서 리스트, 집합, 딕셔너리는 원소로 허용이 안됨.
    - 교집합(& and), 합집합(| or), 차집합(-) 가능. a & b, a | b, a - b
    - emp_set = set() 으로 빈 셋 만들기 가능 ( { } 로 하면 딕셔너리 취급이라서 )
    ...
    print(a & b) # 30, 40, 50만 나옴
    print(a | b) # 10 ~ 70까지 다 나옴
    print(a - b) # 10, 20만 나옴
    ```
---
* 반복문 for, while
